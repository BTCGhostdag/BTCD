...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of BTCD instances
	BTCD1, BTCD2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := BTCD1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In BTCD2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, BTCD2)
	}

	// Connect the two BTCDs. This should trigger sync
	// between the two nodes
	connect(t, BTCD1, BTCD2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := BTCD1.rpcClient.GetVirtualSelectedParentChainFromBlock(

...
import (
	...
	"github.com/BTCGhostdag/BTCD/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		BTCDMessage, err := client.Post(&protowire.BTCDdMessage{Payload: &protowire.BTCDdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := BTCDMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	BTCDMessage := &protowire.BTCDdMessage{}
	err := protojson.Unmarshal([]byte(response), BTCDMessage)
	if err != nil {
	...
	return marshalOptions.Format(BTCDMessage)
}

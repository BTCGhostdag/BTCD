...
// 2. A wrapper that implements isBTCDdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `BTCDdMessage_` prefix and `Request` suffix
...
// generateBTCDdMessage generates a wrapped BTCDdMessage with the given `commandValue`
func generateBTCDdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.BTCDdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	BTCDMessage := reflect.New(reflect.TypeOf(protowire.BTCDdMessage{}))
	BTCDMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return BTCDMessage.Interface().(*protowire.BTCDdMessage), nil
}

...
import (
	...
	"github.com/BTCGhostdag/BTCD/domain/consensus/utils/mining"
	"github.com/BTCGhostdag/BTCD/util"
	"github.com/kaspanet/go-secp256k1"

	"github.com/BTCGhostdag/BTCD/stability-tests/common"
	"github.com/BTCGhostdag/BTCD/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("BTCD-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("BTCD datadir: %s", dataDir)

	BTCDCmd, err := common.StartCmd("KASPAD",
		"BTCD",
	...
	spawn("startNode-BTCDCmd.Wait", func() {
		err := BTCDCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("BTCDCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes BTCD closes ungracefully
				log.Errorf("BTCDCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(BTCDCmd, "BTCDCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	BTCDminerCmd, err := common.StartCmd("MINER",
		"BTCDminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(BTCDminerCmd, "BTCDminerCmd")
	return nil
}

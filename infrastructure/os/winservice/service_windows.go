...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// doneChan. startedChan is notified once BTCD is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("BTCDMain-windows", func() {
...
// logServiceStart logs information about BTCD when the main server has
